...
module.exports = app => {
    ...
    const getByCategory = async (req, res) => {
        const categoryId = req.params.id //categoria pai
        const page = req.query.page || 1
        const categories = await app.db.raw(queries.categoryWithChildren, categoryId)
        const ids = categories.rows.map(c => c.id) //ids filhas

        app.db({ a: 'articles', u: 'users' })
            .select('a.id', 'a.name', 'a.description', 'a.imageUrl', { author: 'u.name' })
            .limit(limit).offset(page * limit - limit)
            .whereRaw('?? = ??', ['u.id', 'a.userId']) //pegar o userId de articles e q seja o mesmo de user
            .whereIn('categoryId', ids) //buscar somente os ids que sao filhas desse pai
            .orderBy('a.id', 'desc')
